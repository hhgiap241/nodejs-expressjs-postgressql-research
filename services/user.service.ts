...
import {UserTable} from "./db_table";
import UserEmailExistedError from "../http-error/UserEmailExistedError";

const getAllUsers = async (): Promise<User[]> => {
  // convert to user model
  const usersTable = await Users.query().select().from('user').withGraphFetched('channel');
  return usersTable.map(convertToModelObject);
}

const getUserById = async (id: string): Promise<User> => {
  const findUser = await Users.query().findById(id).withGraphFetched('channel');
  if (!findUser) throw new UserNotFoundError(404, "User doesn't exist");
  return convertToModelObject(findUser);
}

const insertUser = async (user: User): Promise<User> => {
  const userTable = convertToDbObject(user);
  const findUser = await Users.query().select().from('user').where('email', '=', userTable.email).first();
  if (findUser)
...
const convertToDbObject = (user: User): UserTable => {
...
const convertToModelObject = (userTable: UserTable): User => {
  const user: User = {
    id: userTable.id,
    name: userTable.name,
